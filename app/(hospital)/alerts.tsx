    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={{ height: 90, marginBottom: 8 }}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, gap: 12 }}>
          ...
          {renderStatsCard('Accidents', stats.accidents, '#f97316', 'medkit')}
        </ScrollView>
      </View>
      ...
      <View style={styles.filterTabs}>
        {(['all', 'sos', 'alert'] as const).map((f) => (
          <TouchableOpacity
            ...
            style={[
              styles.filterTab,
              filter === f && { backgroundColor: f === 'sos' ? '#a855f7' : f === 'alert' ? '#f97316' : '#ef4444' }
            ]}
            onPress={() => setFilter(f)}
          >
              ...
              <View style={styles.alertInfo}>
                ...
                <View style={styles.actionRow}>
                  {item.status !== 'resolved' && (
                    <TouchableOpacity
                      style={[styles.actionBtn, { backgroundColor: item.status === 'pending' ? '#ef4444' : '#10b981' }]}
                      onPress={() => handleUpdateStatus(item._id, item.status)}
                    >
                      <Text style={styles.actionBtnText}>
                        {item.status === 'pending' ? 'Treat' : 'Resolve'}
                      </Text>
                    </TouchableOpacity>
                  )}
                  <TouchableOpacity
